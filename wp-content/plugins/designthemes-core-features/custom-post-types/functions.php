...
function dt_set_commission($teacher_id = '') {
	...
	foreach($courses as $course) {
		$starting_price = get_post_meta($course->ID, 'starting-price', true);
...
function dt_pay_commission($teacher_id = '') {
	...
	$out .= '<table border="0" cellpadding="0" cellspacing="20">
				<thead>
				  <tr>
					...
					<th scope="col">'.__('Price', 'dt_themes').'</th>
					<th scope="col">'.__('Commission (%)', 'dt_themes').'</th>
					<th scope="col">'.sprintf(__('To Pay (%s)', 'dt_themes'), dttheme_option('dt_course','currency')).'</th>
	...
	$out .= '<label><strong>'.sprintf(__('Total (%s)', 'dt_themes'), dttheme_option('dt_course','currency')).'</strong></label><input type="text" name="total_amount" id="total-amount" value="0" readonly="readonly" />';
	$out .= '</div>';
