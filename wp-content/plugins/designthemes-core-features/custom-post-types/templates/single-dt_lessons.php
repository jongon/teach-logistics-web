			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('dt-sc-lesson-single'); ?>>
				...
				<div class="lesson-metadata">
				  ...
				  <?php
					...
					<?php 
						$duration = isset($lesson_settings['lesson-duration']) ? $lesson_settings['lesson-duration'] : 0; 
						if($duration > 0) {
				...
				if(isset($lesson_video) && $lesson_video != '') { ?>
					...
					<div class="lesson-video"><?php if(wp_oembed_get( $lesson_video ) != '') echo wp_oembed_get( $lesson_video ); else echo apply_filters( 'the_content', $lesson_video ); ?></div>
				<?php } ?>
               ...
               <div class="dt-sc-hr-invisible-small"></div>
                ...
                if(isset($dt_lesson_quiz) && $dt_lesson_quiz > 0) {
                    ...
                    if(current_user_can($s2_level) || (is_user_logged_in() && $private_lesson == '')) {
                ...
                <div class="dt-sc-lessons-menu">
					...
					if(isset($lessons_array) && count($lessons_array) > 0) {
						...
						foreach ($lessons_array as $lesson_item){
							...
							$duration = isset($lesson_datas['lesson-duration']) ? $lesson_datas['lesson-duration'] : 0; 
							if($duration > 0) {
								$hours = floor($duration/60); 
								$mins = $duration % 60; 
								if($hours == 0) {
