		...
		if(isset($payment_data) && is_array($payment_data)) {
			...
			$out .= '<table border="0" cellpadding="0" cellspacing="20">
					  <tr>
						...
						<th scope="col" class="aligncenter">'.__('Amount ('.dttheme_option('dt_course','currency').')', 'dt_themes').'</th>
					  </tr>';
			...
			if($total > 0) {
				...
				$out .= '<tr>
							...
							<td colspan="5" class="alignright"><strong>'.__('Total ('.dttheme_option('dt_course','currency').')', 'dt_themes').'</strong></td>
							<td>'.$total.'</td>
						</tr>';
